...
def main():
    ...
    #-----------------Maipo
    ruta_Maipo_mon = ruta_OD + r'\datos\RIO MAIPO_Q_mensual.xlsx'
    ruta_Maipo_mon_Mapocho = ruta_OD + r'\datos\RIO MAIPO_Q_mensual_Mapocho_RN.csv'
    ruta_DAA_Mapocho = ruta_OD + r'\datos\DAA_mapocho_MAA_EAM_YLAJM.csv'
    ruta_DAA_Maipo = ruta_OD + r'\datos\DAA_Maipo_RVQ_ROJC_RCRO_RMLM_RMLH.csv'
    ruta_VQ_RN = ruta_OD+r'\datos\Q_Volcan_Queltehues_RN.xlsx'
    ruta_MM_RN = ruta_OD+r'\datos\Q_MM_RN.xlsx'
    ruta_MSA_RN = ruta_OD+r'\datos\Q_Maipo_San_Alfonso_RN.xlsx'
    ruta_ORC_RN = ruta_OD+r'\datos\Q_Olivares_rio_Colorado_RN.xlsx'
    ruta_CRO_RN = ruta_OD+r'\datos\Q_Colorado_Olivares_RN.xlsx'
    ruta_RMLM_RN = ruta_OD+r'\datos\Q_Maipo_Melosas_RN.xlsx'
    ruta_CAJM_RN = ruta_OD+r'\datos\Q_Colorado_Maipo_NAT.xlsx'
    ruta_Maipo_VIC_mon = ruta_OD+r'\datos\VIC\Maipo\q_Maipo_VIC_mon.csv'
    ...
    ruta_DAA_Maule= '..\\..\\SIG\\DAA\\VII\\DAA_20210304 Maule.shp'
    ...
    q_Maipo_mon = pd.read_excel(ruta_Maipo_mon, parse_dates = True, index_col = 0, sheet_name = 'data')
    q_Maipo_flags = pd.read_excel(ruta_Maipo_mon, parse_dates = True, index_col = 0, sheet_name = 'info data')
    q_flags_aux = q_Maipo_flags.copy()
    q_flags_aux = copyIndex(q_flags_aux)
    q_Maipo_mon[q_Maipo_flags < 20] = np.nan #Caudales mensuales con flags
    q_Maipo_mon.drop(['Ano','Mes'], axis=1, inplace=True)
    q_MLA = pd.DataFrame(q_Maipo_mon['05722002-3'])
    q_EYLAJSN = pd.DataFrame(q_Maipo_mon['05721001-K'])
    q_EAM = pd.DataFrame(q_Maipo_mon['05722001-5'])
    ...
    dict_DAA = {'05710001-K' : ['ND-1306-1132', 'ND-1306-5', 'ND-1306-6', 'UA-1306-814244', 'UA-1306-814243'], '' : '', '05721001-K' : ['UA-1306-814243']}
    DAA_Mapocho = pd.read_csv(ruta_DAA_Mapocho)
    
    DAA_Mapocho = DAA_Mapocho[(DAA_Mapocho['Tipo Derec'] == 'Consuntivo') & (DAA_Mapocho['Ejercici_2'] == 'Continuo')]
    for ind,col in q_MLA.iterrows():
        mes_DAA = list(dicc_DAA.keys())[ind.month-1]
        DAA_fecha = DAA_Mapocho.copy()[pd.to_datetime(DAA_Mapocho['Fecha de R']) < ind]
        DAA_fecha_MLA = DAA_fecha.copy()[(DAA_fecha['Estacion'] == '05710001-K') | (DAA_fecha['Estacion'] == '05721001-K')]
        DAA_fecha_EYLAJSN = DAA_fecha.copy()[DAA_fecha['Estacion'] == '05721001-K']
        DAA_fecha_EAM = DAA_fecha.copy()[DAA_fecha['Estacion'] == '05722001-5']

        q_derechos_MLA = DAA_fecha_MLA[mes_DAA].dropna().sum()/1000.
        q_derechos_EYLAJSN = DAA_fecha_EYLAJSN[mes_DAA].dropna().sum()/1000.
        q_derechos_EAM = DAA_fecha_EAM[mes_DAA].dropna().sum()/1000.

        if ind.year > 2011:
            q_derechos_MLA = DAA_fecha_MLA[mes_DAA].dropna().replace(500,700).sum()/1000. #Posterior a 2011 Anglo utiliza los derechos eventuales
            q_derechos_EYLAJSN = DAA_fecha_EYLAJSN[mes_DAA].dropna().replace(500,700).sum()/1000. #Posterior a 2011 Anglo utiliza los derechos eventuales
        
        q_MLA.loc[ind] += q_derechos_MLA
        q_EYLAJSN.loc[ind] += q_derechos_EYLAJSN
        q_EAM.loc[ind] += q_derechos_EAM
    
    q_Maipo_mon['05722002-3'] = q_MLA
    q_Maipo_mon['05721001-K'] = q_EYLAJSN
    q_Maipo_mon['05722001-5'] = q_EAM
