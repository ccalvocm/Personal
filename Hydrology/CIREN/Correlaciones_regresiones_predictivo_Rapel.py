...
#%%    
    ...
    # rutas caudales
    ruta_Q = ruta_OD+r'\Of hidrica\AOHIA_ZC\Etapa 1 y 2\datos\Q_mon_RR_flags.csv'
    ruta_shp = ruta_OD+r'\Of hidrica\GIS\estaciones_Rapel.shp'
    ruta_q_obs = ruta_OD+r'\Of hidrica\AOHIA_ZC\Etapa 1 y 2\datos\RIO RAPEL_Q_mensual.xlsx'
  ...
  #%%Crear indice de fechas, convertir años a int y calcular frecuencia de datos
    ...
    Q_monthly_filtradas = Q_monthly[estaciones_minimas.index].copy()
    
    estaciones_minimas = list(estaciones_minimas.index)
    ...
    estaciones_naturalizadas = ['06008005-4', '06013001-9', '06011001-8','06015001-K','06018001-6',
                                '06033011-5', '06043001-2','06028001-0','06027001-5','06006001-0','06000003-4',
                                '06002001-9','06003001-4']
    ...
    q_obs = q_obs[q_obs_flags > 20]
    q_obs = q_obs[(q_obs.index <= '2020-03-01') & (q_obs.index >= '1979-04-01')]
    estaciones_obs = ['06034022-6','06033011-5','06043001-2','06019003-8']
    ...
    for ind,col in enumerate([x for x in estaciones if x not in ['06019003-8']]):
...
#            
    ...
    for est in estaciones_obs:
        for mes in meses:
            ...
            if est == '06019003-8':
                x['obs'] = Q_monthly_MLR.loc[Q_monthly_MLR.index.month == mes,'06008005-4'].copy()
            else:
            ...
            # Q_monthly_MLR.loc[Q_monthly_mes.index,est] = Q_monthly_MLR.loc[Q_monthly_mes.index,est].fillna(Q_monthly_MLR.loc[Q_monthly_mes.index,est].median())

    # Q_monthly_filtradas = Q_monthly_MLR.copy()
    ...
    Q_monthly_MLR.to_csv(ruta_OD+'\Of hidrica\AOHIA_ZC\Etapa 1 y 2\datos\Q_relleno_MLR_Rapel_'+str(year_i+1)+'-'+str(year_f)+'_monthly_TUR.csv')
