...
folderpath = os.path.join('.', 'outputs', 'validacion_prod_satelital')
...
for column in stations_df.columns:
    ...
    try:
        ...
        # plt.close()
        pass
    filename = os.path.join(folderpath, str(column) + '.' + ext_img)
...
#%% Clip DEM raster for plot

# shapes = catchment_gdf['geometry'].values
# path = os.path.join('..',
#                     'Etapa 1 y 2',
#                     'DEM',
#                     'DEM Alos 5a a 8a mar.jp2')
...
for station in df_validation.index:
    ...
    ax1.annotate(text=s1, xy=(x,y), ha='center', xytext = (xtext,ytext), 
                 arrowprops=dict(facecolor='black', shrink=0.05, width = 0.1,
                                 headwidth = 0.1))
    ax2.annotate(text=s2, xy=(x,y), ha='center', xytext = (xtext,ytext))
    ax3.annotate(text=s3, xy=(x,y), ha='center', xytext = (xtext,ytext))
...
for index in df_validation.index:
    df_validation.loc[index, 'x'] = stations_gdf.loc[index, 'londd']
    df_validation.loc[index, 'y'] = stations_gdf.loc[index, 'latdd']
    
gdf_validation = gpd.GeoDataFrame(df_validation, geometry=gpd.points_from_xy(df_validation.x, 
                                                                             df_validation.y),
                                  crs = 'EPSG:4326')
gdf_validation = gdf_validation.to_crs('EPSG:32719')
catchment_gdf = catchment_gdf.to_crs('EPSG:32719')
...
vmin = df_validation[['NSE-CDR', 'NSE-CCS', 'NSE-CR2', 'NSE-GPM']].min().min()
vmax = df_validation[['NSE-CDR', 'NSE-CCS', 'NSE-CR2', 'NSE-GPM']].max().max()
vmin = -1
vmax = 1
...
for a, col in zip(ax.flat,['NSE-CDR', 'NSE-CCS', 'NSE-CR2', 'NSE-GPM']):
    catchment_gdf.plot(ax=a, fc='none', ec='red', lw=0.2)
    gdf_validation.plot(ax=a, column = col, legend=False,
                        cmap=cmap, norm = norm)
    divider = make_axes_locatable(a)
    cax = divider.append_axes("right", size="5%", pad=0.5)
    cbar = fig.colorbar(sm, cax=cax, shrink=1.0)
    cbar.ax.set_ylabel('NSE')
    a.set_xlabel('Coordenada Este UTM (m)')
    a.set_ylabel('Coordenada Norte UTM (m)')
    a.set_title(col)

filename = os.path.join(folderpath, 'NSE_x_producto.jpg')    
plt.savefig(filename, format=ext_img, bbox_inches = 'tight',
                pad_inches = 0)