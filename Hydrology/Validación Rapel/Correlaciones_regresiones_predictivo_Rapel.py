...
#%%    
    ...
    ruta_Q = ruta_OD+r'\Of hidrica\AOHIA_ZC\Etapa 1 y 2\datos\RIO RAPEL_Q_mensual.xlsx'
    ruta_shp = ruta_OD+r'\Of hidrica\GIS\estaciones_Rapel.shp'

    est_geo = geopandas.read_file(ruta_shp)

    Q_monthly = pd.read_excel(ruta_Q, index_col = 0, parse_dates = True, sheet_name = 'data')
    ...
    Q_monthly = Q_monthly.loc[Q_monthly.index <= fin ]  
  ...
  #%%Crear indice de fechas, convertir años a int y calcular frecuencia de datos
    ...
    Q_monthly_filtradas = Q_monthly_filtradas[Q_monthly_filtradas.columns[:-2]]
...
#        
    ...
    Q_monthly_MLR.to_csv('Q_relleno_MLR_Rapel_'+str(year_i+1)+'-'+str(year_f)+'_monthly_OBS.csv')
