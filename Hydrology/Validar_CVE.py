...
# URL: https://mma.gob.cl/wp-content/uploads/2017/12/Maipo.pdf
  ...
  for i,estacion in enumerate(estaciones_date):
    ...
    for index, col in enumerate(probabilidades_excedencia):
        ...
        CVE_sinr = CVE(pd.DataFrame(Q[estacion].loc[(Q.index <= fechas[-1]) & (Q.index >= fechas[0])], columns = [estacion], index = Q.index), probabilidades_excedencia)[str(probabilidades_excedencia[index])][estacion]
       
        pbb_mensuales.loc[pbb_mensuales.index, col] =  CVE_rellenada.to_list()
        
        pbb_mensuales_sinr.loc[pbb_mensuales_sinr.index, col] =  CVE_sinr.to_list()

        
    caudales_pbb_mes[estacion] = pbb_mensuales
    caudales_pbb_mes_sinr[estacion] = pbb_mensuales_sinr
...
#Graficar
    ...
    caudales_pbb_mes_sinr[estacion].plot(ax = axis, color = 'b', legend=False, linewidth = 3)
